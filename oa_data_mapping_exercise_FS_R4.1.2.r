...
oa_religion_data = read_csv("data/OA data/TS030-2021-1-filtered-2022-12-12T13_36_57Z.csv") %>%
  clean_names()

oa_ethnicity_data = read_csv("data/OA data/TS021-2021-1-filtered-2022-12-18T18_58_37Z.csv") %>%
...
oa_lsoa_mapping = read_csv("data/OAs_to_LSOAs_to_MSOAs_to_LEP_to_LAD_(December_2022)_Lookup_in_England.csv") %>%
  clean_names()
...
draw_map = function(ltla_list, level, variable){
  # restrict data to only those local authorities requested eg. change filter to filter(ltla22nm %in% c("Harrow","Brent"))
  ...
  if(level == "ward"){
    ...
    area_names = "wpc22nm" #FS add
    map = pcon_map %>% 
      inner_join(agg_oa_data, by = c("PCON21CD"="wpc22cd"))
  }
  
  # use the shapefile to create the map using tmap
  map = tm_shape(map) +
    ...
    tm_text(text = area_names, size = 0.7)+
...
draw_map(ltla_list = c("Hillingdon"), level = "ward", variable = "muslim")
...
draw_map(ltla_list = c("Manchester"), level = "pcon", variable = "muslim")
draw_map(ltla_list = c("Manchester"), level = "ward", variable = "muslim")
